...
def vector_graph_qa(query):
    ...
    WITH product, s, c, count(*) AS orderCount
    WITH product, 
    ...
    WITH product, productSupplierName, totalOrders, customerData, recommendedProduct, count(*) AS copurchaseCount
    WHERE copurchaseCount > 2
    RETURN  product.text AS product, 
        productSupplierName, 
            totalOrders, 
            customerData,
